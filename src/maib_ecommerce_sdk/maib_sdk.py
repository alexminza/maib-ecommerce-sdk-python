...
class MaibSdk:
    ...
    def _process_response(self, response: httpx.Response):
        if response.is_error:
            logger.error('%s Error: %d %s', self.__class__.__qualname__, response.status_code, response.text, extra={'method': response.request.method, 'url': response.request.url, 'response_text': response.text, 'status_code': response.status_code})
    ...
    def send_request(self, method: str, url: str, data: dict = None, token: str = None, entity_id: str = None):
        ...
        logger.debug('%s Request: %s %s', self.__class__.__qualname__, method, url, extra={'method': method, 'url': url, 'data': data, 'token': token})
